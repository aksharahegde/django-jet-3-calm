...
(function ($) {
  ...
  if (!$.escapeSelector) {
    $.escapeSelector = function (sel) {
      ...
      // Try to use jQuery's internal mechanism if available (jQuery 3+ has escapeSelector built-in)
      // Since jQuery 3+ should have escapeSelector, if we reach here with jQuery 3+,
      // it means escapeSelector wasn't available, so we implement manually

      // Manual spec-compliant CSS.escape implementation
      var string = String(sel);
      var length = string.length;
      var index = -1;
      var codeUnit;
      var result = "";
      var firstCodeUnit = length > 0 ? string.charCodeAt(0) : 0;

      while (++index < length) {
        codeUnit = string.charCodeAt(index);

        // NULL character (U+0000) -> U+FFFD
        if (codeUnit === 0x0000) {
          result += "\uFFFD";
          continue;
        }

        // ASCII control characters (U+0001 to U+001F) and DEL (U+007F)
        if ((codeUnit >= 0x0001 && codeUnit <= 0x001f) || codeUnit === 0x007f) {
          result += "\\" + codeUnit.toString(16).toUpperCase() + " ";
          continue;
        }

        // Leading digit (U+0030 to U+0039)
        if (index === 0 && codeUnit >= 0x0030 && codeUnit <= 0x0039) {
          result += "\\" + codeUnit.toString(16).toUpperCase() + " ";
          continue;
        }

        // Leading hyphen (U+002D)
        if (index === 0 && codeUnit === 0x002d) {
          result += "\\" + codeUnit.toString(16).toUpperCase() + " ";
          continue;
        }

        // Second character: if first is hyphen, escape if second is digit or hyphen
        if (index === 1 && firstCodeUnit === 0x002d) {
          if (
        ...
        // High surrogate (U+D800 to U+DBFF) followed by low surrogate (U+DC00 to U+DFFF)
        if (codeUnit >= 0xd800 && codeUnit <= 0xdbff) {
          if (index + 1 < length) {
            var nextCodeUnit = string.charCodeAt(index + 1);
            if (nextCodeUnit >= 0xdc00 && nextCodeUnit <= 0xdfff) {
              var codePoint =
                0x10000 + (codeUnit - 0xd800) * 0x400 + (nextCodeUnit - 0xdc00);
  ...
  $.extend(Timepicker.prototype, {
    ...
    _generateHTML: function (inst) {
      var h,
        ...
        deselectButtonText = DOMPurify.sanitize(
          this._get(inst, "deselectButtonText")
        ),
        showButtonPanel =
...
*/
    ...
    _generateHTMLMinuteCell: function (inst, minute, displayText) {
      ...
      if (onMinuteShow) {
        ...
        enabled = onMinuteShow.apply(inst.input ? inst.input[0] : null, [
          inst.hours,
          minute,
        ]); //trigger callback
      }
      ...
      if (enabled && hour !== null) {
        if (
          !isNaN(parseInt(maxTime.hour)) &&
          !isNaN(parseInt(maxTime.minute)) &&
          hour >= maxTime.hour &&
          minute > maxTime.minute
        )
          enabled = false;
        if (
          !isNaN(parseInt(minTime.hour)) &&
          !isNaN(parseInt(minTime.minute)) &&
          hour <= minTime.hour &&
    ...
    _setTimeFromField: function (inst) {
      ...
      var timeToParse =
        defaultTime == "now" ? this._getCurrentTimeRounded(inst) : defaultTime;
    ...
    _getCurrentTimeRounded: function (inst) {
      var currentTime = new Date(),
        ...
        adjustedMinutes =
          Math.round(currentMinutes / minutes_options.interval) *
          minutes_options.interval;
    ...
    selectNow: function (event) {
      var idAttr = $(event.target).attr("data-timepicker-instance-id"),
        rawId = idAttr ? idAttr.replace(/^#/, "") : null,
        targetElem = rawId
          ? document.getElementById(rawId) ||
            $("#" + $.escapeSelector(rawId))[0]
          : null,
        $target = targetElem ? $(targetElem) : $([]),
    ...
    selectHours: function (event) {
      var $td = $(event.currentTarget),
        idAttr = $td.attr("data-timepicker-instance-id"),
        rawId = idAttr ? idAttr.replace(/^#/, "") : null,
